...
static void callback(uint64_t begin, int *values, int count) {
    if (fd < 0) return;
...
//    int *out = static_cast<int *>(malloc(sizeof(int) * count));
//    memcpy(out, values, count);
    cJSON_AddItemToObject(obj, "begin", cJSON_CreateNumber((double) begin));
    cJSON_AddItemToObject(obj, "values", cJSON_CreateIntArray(values, count));
...
static void spectrumThread(void *arg) {
